...
- **Telemetry never carries payloads.**
  - Logs: field allowlist in `@prysm/platform` and in the ai-service JSON formatter. Unknown fields are dropped and only their key names kept. Tests use a PII canary.
  - Traces: query strings are redacted before export. Found in M0: the HTTP span recorded `url.query` and put a canary into Tempo. Fixed by `RedactingSpanProcessor` (TS, SDK `onEnding` hook) and a FastAPI `server_request_hook` (Python).
  - `scripts/verify-m0.mjs` asserts the canary is absent from both Loki and Tempo, end to end.
  - Request logs record route templates, never raw URLs. The uvicorn access log is disabled.
- **Supply chain** (F-06), enforced in CI by Semgrep:
  - pnpm `minimumReleaseAge` 7 days, `trustPolicy: no-downgrade`, `blockExoticSubdeps`, `strictDepBuilds` with a reviewed `allowBuilds` list.
  - uv `exclude-newer = "7 days"`, frozen lockfiles, and a hash-pinned `packageManager`.
  - Actions pinned by SHA. The `GITHUB_TOKEN` is read-only by default.
  - gitleaks (reviewed false positives go in `.gitleaksignore` by fingerprint), OSV, dependency review, Semgrep `p/default`, Trivy on every image, CycloneDX SBOM, and cosign keyless signing on `main`.
- **Runtime images** run as non-root, and the unused npm/yarn/corepack CLIs are removed from them. The migrate image carries only `@prysm/db` and drizzle-kit.
- **Dev stack:**
  - Every port binds to `127.0.0.1`.
  - Credentials are dev-only and labelled so; production roles and secrets come from infrastructure (TRACKING F-43).
  - Runtime DB roles are `NOSUPERUSER NOBYPASSRLS` and own nothing. Only the owner can create objects in `public`, and `baseline.test.ts` proves it.
  - The SeaweedFS app identity cannot administer buckets; a separate admin identity is used only by the one-shot bucket init.

**New trust-boundary notes**

- `client.address` (a caller IP) is recorded on server spans. That's acceptable for security telemetry, but it is personal data under DPDP/GDPR, and trace retention must be set accordingly (M12 SLOs/retention).
- **Open:** OTel HTTP instrumentation captures no headers by default. Any future header capture must go through the same redaction review.
