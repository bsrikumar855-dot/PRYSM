...
- Postgres: `prysm_app` gets `INSERT, SELECT` only on `evidence_records` and `evidence_anchors`. A `BEFORE UPDATE OR DELETE` trigger raises an exception unless the role is `prysm_platform` *and* the operation is retention expiry, which may only null out `payload_ref`. Rows are never deleted.
...
- **Transparency log (Trillian / Sigstore Rekor) as the anchor target:** a strong option for later. It's an external service, so it's listed as optional future work.
