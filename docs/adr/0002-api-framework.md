...
This is "schema-first", not hand-written "OpenAPI-first". A hand-written YAML spec and its TS types drift apart. With zod as the single source, runtime validation, static types and the spec can't disagree, and the committed spec is still the reviewed, diffable contract. **Needs your approval.**
