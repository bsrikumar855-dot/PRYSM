...
- **Benchmark gate:** autocannon/k6 scenarios in `apps/gateway/bench` run against a mock upstream with fixed latency. CI measures *added* latency (gateway minus mock) at 500 RPS on a pinned runner size and fails if p95 > 30 ms. The runner spec is recorded with the results.
