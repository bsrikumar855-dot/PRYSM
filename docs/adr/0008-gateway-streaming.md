...
- **Benchmark gate:** autocannon/k6 scenarios in `apps/gateway/bench` run against a mock upstream with fixed latency. The benchmark measures *added* latency (gateway minus mock) at 500 RPS and fails if p95 > 30 ms. The runner spec is recorded with the results. There is no dedicated runner yet (owner, 2026-09-24), so the gate runs as a **blocking nightly job** and as a **non-blocking check on PRs**, which reports a regression without failing. When a dedicated runner exists, the PR check becomes blocking.
