...
Status: **Proposed** (v1, 2026-09-24). Decisions referenced as ADR-000N live in `docs/adr/`.
...
- **SaaS:** AWS first. Per region: EKS, RDS Postgres 16 (pgvector), ElastiCache Redis (AOF, `noeviction`), S3 with Object Lock, KMS. Terraform in `infra/terraform`, workloads via the Helm chart.
...
- **PRYSM's own LLM calls:** a single adapter in the ai-service. Each call logs provider, model, template id@version, SHA-256 of the inputs, output, token counts and latency, as an evidence record.
