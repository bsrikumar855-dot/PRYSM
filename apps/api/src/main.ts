...
const pool = createPool({ connectionString: config.DATABASE_URL_APP, applicationName: 'prysm-api' });
...
const { app, markShuttingDown } = buildApi({ logger, pool, valkey });
...
logger.info({ event: 'started', port: config.PORT }, 'api listening');
