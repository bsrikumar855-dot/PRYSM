...
describe('loadConfig', () => {
  it('applies defaults and coerces numbers', () => {
    ...
    expect(cfg).toMatchObject({ SERVICE_NAME: 'api', PORT: 4000, HOST: '0.0.0.0', LOG_LEVEL: 'info', SHUTDOWN_DRAIN_DELAY_MS: 0 });
  });

  it('lists every problem and refuses to start', () => {
    expect(() => loadConfig(baseEnv, { PORT: '70000', LOG_LEVEL: 'loud' })).toThrow(/SERVICE_NAME[\s\S]*PORT[\s\S]*LOG_LEVEL/);
  });
